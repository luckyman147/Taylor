...
describe('api client', () => {
  ...
  describe('API_BASE / getUploadUrl', () => {
    ...
    it('appends as_master=true when uploading as a master resume', () => {
      expect(getUploadUrl(true)).toBe('/api/v1/resumes/upload?as_master=true');
      expect(getUploadUrl(false)).toBe('/api/v1/resumes/upload');
    });
  });
