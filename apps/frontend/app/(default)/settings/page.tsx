...
export default function SettingsPage() {
  ...
  const handleProviderChange = (newProvider: LLMProvider) => {
    ...
    // (each provider keeps its own key ΓÇö switching no longer wipes anything).
    setApiKey('');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // (2) Persist non-secret LLM config ΓÇö WITHOUT api_key.
      const update: LLMConfigUpdate = {
        provider,
        model: model.trim(),
        api_base: apiBase.trim() || null,
        // Map UI sentinel 'auto' ΓåÆ '' so the server persists an empty string
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-start overflow-y-auto pl-16 p-4 md:p-8">
      <SidebarNav currentPage="settings" onNavigate={(page) => router.push(page)} />
              ...
              {activeSection === 'system' && (
                <section className="space-y-4">
                  ...
                  {statusLoading ? (
                    ...
                    // rather than the viewport ΓÇö useful when the settings page is
              ...
              {activeSection === 'ai' && (
                <section className="space-y-6">
                  ...
                  <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    {/* API Key Input ΓÇö always enabled. For providers that don't
                  ...
                  fails when `requiresApiKey` is true. */}
                    ...
                    {/* Saved per-provider keys ΓÇö each provider keeps its own encrypted
