    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-[400px] flex flex-col items-center justify-center p-8 bg-background">
          <div className="max-w-md w-full bg-white border border-ink shadow-sw-default p-8">
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="w-8 h-8 text-red-600" />
              <h2 className="font-serif text-2xl font-bold uppercase">{strings.title}</h2>
            </div>

            <p className="text-ink-soft mb-4  text-sm">{strings.description}</p>

            {process.env.NODE_ENV === 'development' && this.state.error && (
              <div className="mb-4 p-3 bg-[#fdf3f2] border border-red-200 rounded-lg">
            ...
            <div className="flex gap-3">
              <Button
                ...
                variant="outline"
                className="flex-1 border-ink rounded-lg shadow-sw-sm hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all"
              >
                {strings.tryAgain}
              </Button>
              <Button
                onClick={this.handleReload}
                className="flex-1 bg-primary hover:bg-[#17304f] text-white rounded-lg border border-ink shadow-sw-sm hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all"
              >
