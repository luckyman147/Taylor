...
export const ResumeLatex: React.FC<ResumeLatexProps> = ({
  ...
  const renderContactDetail = (label: string, value?: string, hrefPrefix: string = '') => {
    ...
    if (
      ['Website', 'Portfolio', 'LinkedIn', 'GitHub'].includes(label) &&
    ...
    const displayText = isLink ? label : value;
  ...
  const contactItems = [
    ...
    renderContactDetail('Portfolio', personalInfo?.website),
  ].filter(Boolean);
            ...
            <div className={baseStyles['resume-items']}>
              {workExperience.map((exp) => (
                <div key={exp.id} className={baseStyles['resume-item']}>
                  {renderEntryHeader(exp.title, exp.years, exp.company, exp.location)}
            ...
            <div className={baseStyles['resume-items']}>
              {education.map((edu) => (
                <div key={edu.id} className={baseStyles['resume-item']}>
                  {renderEntryHeader(edu.degree, edu.years, edu.institution)}
                  {edu.description && (
