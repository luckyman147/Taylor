...
export function GeneratePrompt({
  ...
  if (!isTailoredResume) {
    return (
      ...
      >
        ...
        <p className=" text-xs text-steel-grey max-w-md mb-6 leading-relaxed">
  ...
  return (
    ...
    >
      <div className="w-16 h-16 border-2 border-primary bg-primary/5 flex items-center justify-center mb-6">
        <Icon className="w-8 h-8 text-primary" />
      </div>
      <h3 className=" text-sm font-bold uppercase tracking-wider mb-3">
        {t('builder.generatePrompt.generateTitle', { title })}
      </h3>
      <p className=" text-xs text-ink-soft max-w-md mb-6 leading-relaxed">
        {isInterviewPrep
