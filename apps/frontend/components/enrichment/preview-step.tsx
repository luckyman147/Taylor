  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-ink-soft  text-sm">{t('enrichment.preview.description')}</p>
      </div>
...
function EnhancementCard({ enhancement }: EnhancementCardProps) {
  ...
  return (
    <div className="border border-[#e6e3dc] bg-white shadow-sw-default">
      {/* Card header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[#e6e3dc] bg-paper-tint">
        {enhancement.item_type === 'experience' ? (
          <Briefcase className="w-4 h-4" />
        ) : (
          <FolderKanban className="w-4 h-4" />
        )}
        <span className=" text-sm font-bold uppercase">{itemTypeLabel}</span>
