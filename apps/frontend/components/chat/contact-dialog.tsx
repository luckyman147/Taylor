...
export function ContactDialog({ isOpen, onClose, initialData, onCreated }: ContactDialogProps) {
      ...
      <div className="relative mx-4 w-full max-w-md rounded-xl bg-white shadow-2xl">
        ...
        <div className="space-y-3 p-4">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-md border border-[#e6e3dc] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-ink-muted mb-1">Email</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className="w-full rounded-md border border-[#e6e3dc] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-ink-muted mb-1">Company</label>
            <input value={company} onChange={(e) => setCompany(e.target.value)} className="w-full rounded-md border border-[#e6e3dc] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        ...
        <div className="flex justify-end gap-2 border-t border-[#e6e3dc] p-4">
          ...
          <button onClick={handleSave} disabled={saving || !name.trim()} className="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50">
