...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, variant = 'default', noPadding = false, ...props }, ref) => {
    ...
    const variants = {
      default: 'bg-card',
      interactive: cn(
        'bg-card border border-[#e6e3dc] shadow-sm',
        'transition-[box-shadow,border-color] duration-200 ease-in-out',
        'cursor-pointer group',
        'hover:border-ink hover:shadow-sw-default'
      ),
      outline: 'bg-card border border-[#e6e3dc]',
      ghost: 'bg-transparent border-none shadow-none',
    };

    // Dashboard specific style that was common:
    // border-2 border-dashed border-[#d9c9a3] bg-[#fbf6e9]
...
>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn('text-sm text-steel-grey ', className)} {...props} />
));
