 ...
 *
 * ``asMaster`` opts the upload in as a master (base) resume via the
 * ``?as_master=true`` query param — the backend keeps ``false`` as the default
 * (first upload still auto-becomes master).
 */
export function getUploadUrl(asMaster?: boolean): string {
  return asMaster ? `${API_BASE}/resumes/upload?as_master=true` : `${API_BASE}/resumes/upload`;
}
