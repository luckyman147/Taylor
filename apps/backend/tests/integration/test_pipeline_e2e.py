...
class TestPipelineCore:
    ...
    async def test_upload_as_master_creates_second_master(
        self, isolated_db, sample_resume
    ):
        """Uploading with ``?as_master=true`` when a master already exists
        creates a second master instead of a tailored resume."""
        first_resp = await _upload_resume(isolated_db, sample_resume)
        assert first_resp.status_code == 200
        assert first_resp.json()["is_master"] is True
        ...
        with (
            ...
            async with _new_client() as client:
                second_resp = await client.post(
                    "/api/v1/resumes/upload?as_master=true",
                    files={"file": ("resume2.pdf", b"%PDF-1.4 fake", "application/pdf")},
                )

        assert second_resp.status_code == 200
        assert second_resp.json()["is_master"] is True
        masters = [r for r in await isolated_db.list_resumes() if r["is_master"]]
        assert len(masters) == 2

    async def test_upload_without_flag_not_master_after_first(
        self, isolated_db, sample_resume
    ):
        """A plain upload (no ``as_master``) after the first master exists stays
        a tailored resume."""
        await _upload_resume(isolated_db, sample_resume)
        markdown = "# Jane Doe\nSenior Backend Engineer\njane@example.com\n"
        with (
            patch(
                "app.routers.resumes.parse_document",
        ...
        assert second_resp.status_code == 200
        assert second_resp.json()["is_master"] is False
