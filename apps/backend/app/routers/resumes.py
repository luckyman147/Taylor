...
async def upload_resume(
    file: UploadFile = File(...), as_master: bool = Query(False)
) -> ResumeUploadResponse:
    ...
    ``as_master`` marks the upload as a master (base) resume; otherwise the
    first upload auto-becomes master.
    """
    # Validate file type
...
async def _improve_preview_flow(
    *,
    ...
    try:
        # Alignment source of truth: the base resume's own data, or the resume
        # it descends from (``parent_id``) when the base is a tailored copy.
        # Multi-master model — never the single global master.
        alignment_resume = resume
        if resume.get("parent_id"):
            parent_resume = await db.get_resume(resume["parent_id"])
            if parent_resume is not None:
                alignment_resume = parent_resume
        master_data = _get_original_resume_data(alignment_resume)
        if master_data:
...
async def download_resume_pdf(
    ...
    bodyFont: str = Query("sans-serif", pattern="^(serif|sans-serif|mono|times-roman)$"),
